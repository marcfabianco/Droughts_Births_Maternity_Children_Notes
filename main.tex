...
Results for the TWFE model (equation \ref{eq:twfe_regression}) are presented in the following tables for different birth outcomes. Column 1 shows the simpler specification with municipality and year-month fixed effects. Column 2 adds state-specific linear yearly trends to capture differential long-term trends across states. Column 3 further incorporates municipality-specific calendar month fixed effects to control for seasonality in birth outcomes, accounting for systematic patterns linked to specific months with consistent environmental conditions like temperature. Together with state-specific trends, this specification addresses both regional long-term dynamics and local seasonal variations. Finally, Column 4 present the alternative estimator developed in \cite{deChaisemartin2023} which accounts for the potential bias of having several treatments and heterogeneous effects.\footnote{For this version of the document, the alternative estimator is presented only for the most severe drought category due computation time.}
...
\input{tables/TWFE_lbw_results.tex}
...
\subsection{Other birth outcomes}
